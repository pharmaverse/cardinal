...
#' @describeIn make_table_32 returns an object matching the selected `tbl_engine` argument.
#'
#' @return
...
#'
#' @return
#' * `make_table_32_rtables` returns an `rtables` table object.
#' @examples
#' adsl <- random.cdisc.data::cadsl
#' advs <- random.cdisc.data::cadvs
#'
#' tbl <- make_table_32_rtables(advs = advs, alt_counts_df = adsl)
#' tbl
#'
#' @export
...
#' @keywords Internal
make_ard_32 <- function(data = data,
                        by = "ARM",
                        categorical_vars = c("L60", "G60", "G90", "G110", "GE120")) {
