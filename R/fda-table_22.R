#' FDA Table 22: Overview of Adverse Events by Demographic Subgroup, Safety Population,
#'   Pooled Analysis
#'
#' @description
#' Creates FDA Table 22 showing an overview of adverse events by demographic subgroup,
#' including any treatment-emergent AE and counts by subgroup categories.
#'
...
#'
#' @details
#' * `df` must contain the variables specified by `vars`, `arm_var`, `id_var`, `saffl_var`,
#'   and `trtemfl_var`.
...
#' * Numbers represent the absolute number of patients with an AE and fraction of `N`.
...
#'
#' result <- make_table_22(df = df, denominator = adsl, arm_var = "ARM")
#' result$table
#'
...
make_table_22 <- function(df,
  ...
  stopifnot(all(c(id_var, arm_var, saffl_var, trtemfl_var, vars) %in% names(df)))
  ...
  df <- df |>
    dplyr::filter(.data[[saffl_var]] == "Y", .data[[trtemfl_var]] == "Y") |>
    dplyr::mutate(
      ANY_AE = TRUE
    )

  include_vars <- c("ANY_AE", vars)
