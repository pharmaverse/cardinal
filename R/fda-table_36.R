#' FDA Table 36: Patients With Adverse Events by System Organ Class and Preferred Term,
#'   Safety Population, Pooled Analysis (or Trial X)
#'
#' @description
#' Creates FDA Table 36 showing patients with adverse events by system organ class and
...
#'
#' @details
#' * `df` must contain the variables specified by `id_var`, `arm_var`, `saffl_var`, `soc_var`,
#'   and `pref_var`.
...
#'
#' result <- make_table_36(df = adae, denominator = adsl)
#' result$table
#'
...
make_table_36 <- function(df,
  ...
  stopifnot(all(c(id_var, arm_var, saffl_var, soc_var, pref_var) %in% names(df)))
