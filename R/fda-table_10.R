...
#'
#' @description
#' Creates FDA Table 10 showing patients with serious adverse events by system organ class
#' and FDA Medical Query (Narrow or Broad) for the safety population.
#'
...
#' @param fmqsc_var (`character`)\cr name of the FMQ scope variable (contains `"BROAD"` or `"NARROW"`).
#'   Defaults to `"FMQ01SC"`.
...
#'
#' result <- make_table_10(df = adae, denominator = adsl, arm_var = "ARM")
#' result$table
#'
#' @importFrom dplyr filter all_of select distinct
...
make_table_10 <- function(df,
  ...
  stopifnot(all(c("AEBODSYS", "AESER", arm_var, id_var, saffl_var, fmqsc_var, fmqnam_var) %in% names(df)))
