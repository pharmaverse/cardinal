#' FDA Table 8: Patients With Serious Adverse Events by System Organ Class and Preferred Term,
#'   Safety Population, Pooled Analyses
#'
#' @inheritParams argument_convention
#'
#' @name make_table_08_rtables
NULL


#' @describeIn make_table_08_rtables Create FDA table 8 using functions from `rtables` and `tern`.
#'
#' @details
...
#'
#' @return
#' * `make_table_08_rtables` returns an `rtables` table object.
#'
...
#'
#' tbl <- make_table_08_rtables(adae = adae, alt_counts_df = adsl)
#' tbl
#'
#' @export
make_table_08_rtables <- function(adae,
                                  alt_counts_df = NULL,
                                  show_colcounts = TRUE,
                                  id_var = "USUBJID",
                                  arm_var = "ARM",
...
#' @describeIn make_table_08 Create FDA table 8 using functions from `Tplyr`.
#'
...
#'
#' @note
#' * `make_table_08_tplyr()` does not support annotations in case `tplyr_raw = TRUE`.
#' * `make_table_08_tplyr()` raises a warning in case `pref_var` or `soc_var` is a factor. Those can safely be ignored.
#' * `make_table_08_tplyr()` does not explicitly handle missings.
#'
#' @return
#' * `make_table_08_tplyr()` returns a `gt_tbl` object when `tplyr_raw = FALSE` (default) and
...
#'
#' # Basic table
#' make_table_08_tplyr(adae = adae, alt_counts_df = adsl)
#'
#' # Activate risk difference column
#' rd_pairs <- list(c("A: Drug X", "B: Placebo"), c("A: Drug X", "C: Combination"))
#' make_table_08_tplyr(adae = adae, alt_counts_df = adsl, risk_diff_pairs = rd_pairs)
#'
#' # Add overall column
#' make_table_08_tplyr(adae = adae, alt_counts_df = adsl, lbl_overall = "Total patients")
#'
#' # Add titles and footnotes
#' annot <- list(
#'   title = "Table 8. Patients with Serious Adverse Events by SOC and PT, Safety Population,
#'     Pooled Analyses",
...
#' )
#' make_table_08_tplyr(adae = adae, alt_counts_df = adsl, annotations = annot)
#'
#' @export
make_table_08_tplyr <- function(
  adae,
  alt_counts_df = NULL,
  id_var = "USUBJID",
  arm_var = "ARM",
...
#' @describeIn make_table_08 Create FDA table 8 using functions from `gt`.
#'
#' @inheritParams argument_convention
...
#'
#' @return
#' * `make_table_08_gtsum` returns a `gt` object.
#'
#' @examples
#' adsl <- random.cdisc.data::cadsl
#' adae <- random.cdisc.data::cadae
#'
#' tbl <- make_table_08_gtsum(adae = adae, alt_counts_df = adsl)
#' tbl
#'
#' annotations <- list(
#'   title = "Table 8. Patients With Serious Adverse Events",
...
#'
#' @export
make_table_08_gtsum <- function(adae,
                                alt_counts_df = NULL,
                                show_colcounts = TRUE,
                                saffl_var = "SAFFL",
                                ser_var = "AESER",
  ...
  if (!is.null(lbl_overall)) {
    overall_list <- create_table_08_data(
      adae = adae,
      alt_counts_df = alt_counts_df,
      arm_var = arm_var,
      id_var = id_var,
...
#' Helper functions used to construct FDA table 8
#'
#' @inheritParams argument_convention
#' @inheritParams make_table_08_gtsum
#'
#' @seealso [make_table_08()], [make_table_08_tplyr()], [make_table_08_gtsum()]
#'
#' @name h_make_table_08
NULL

#' @describeIn h_make_table_08 Helper function to create the data for [make_table_08_gtsum()].
#'
#' @return
#' * `create_table_08_data` returns a list containing the counted data to be displayed for table 8 and
#'   a `data.frame` containing information about the total N for each group.
#'
#' @details
#' * `create_table_08_data`: If `lbl_overall` is non-`NULL`, only the data for the overall column will be generated.
#'
#' @keywords internal
create_table_08_data <- function(
  adae,
  alt_counts_df,
  arm_var,
  id_var,
...
#' @describeIn h_make_table_08 Helper function used to count subjects per group.
#'
...
#' @describeIn h_make_table_08 Helper function to rename the combined columns.
#'
...
#' @describeIn h_make_table_08 Helper function to calculate the risk difference for table 8.
#'
...
#' Engine-Specific Functions: Table 08
#'
...
#'
#' @return
#' * `make_table_08_gtsummary()` returns a `gtsummary` object.
#'
#' @seealso [make_table_08()]
#'
#' @export
#'
#' @examples
...
#' adae <- random.cdisc.data::cadae
#'
#' tbl_gtsummary <- make_table_08_gtsummary(df = adae, denominator = adsl)
#' tbl_gtsummary
make_table_08_gtsummary <- function(
  df,
  denominator = NULL,
  saffl_var = "SAFFL",
  ser_var = "AESER",
...
#' adae <- random.cdisc.data::cadae
#'
#' tbl <- make_table_08(df = adae, denominator = adsl)
#' tbl
make_table_08 <- function(
  df,
  denominator = NULL,
  return_ard = TRUE,
  id_var = "USUBJID",
