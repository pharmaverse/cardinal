#' FDA Table 17: Patients With Adverse Events by Female-Specific FDA Medical Query (Narrow)
#'   and Preferred Term, Female Safety Population, Pooled Analyses
#'
#' @description
#' Creates FDA Table 17 showing patients with adverse events by female-specific FDA Medical Query
#' (Narrow) and preferred term, restricted to the female safety population.
#'
...
#' @param sex_scope (`character`)\cr sex group to include. Defaults to `"F"` (female).
#' @param fmq_scope (`character`)\cr FMQ scope: `"NARROW"` or `"BROAD"`. Defaults to `"NARROW"`.
...
#' * FMQ names that appear in the non-female population are excluded from the female analysis.
...
#' )
#'
#' result <- make_table_17(df = adae, denominator = adsl, arm_var = "ARM")
#' result$table
#'
#' @importFrom dplyr filter all_of
#' @importFrom gtsummary tbl_hierarchical gather_ard
#' @export
make_table_17 <- function(df,
                           denominator = NULL,
                           return_ard = TRUE,
                           id_var = "USUBJID",
                           arm_var = "TRT01A",
