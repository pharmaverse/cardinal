#' FDA Table 15: Patients With Adverse Events by Male-Specific FDA Medical Query (Narrow)
#'   and Preferred Term, Male Safety Population, Pooled Analyses
#'
#' @description
#' Creates FDA Table 15 showing patients with adverse events by male-specific FDA Medical Query
#' (Narrow) and preferred term, restricted to the male safety population.
#'
...
#' )
#'
#' result <- make_table_15(df = adae, denominator = adsl, arm_var = "ARM")
#' result$table
#'
#' @importFrom dplyr filter all_of
#' @importFrom gtsummary tbl_hierarchical gather_ard
#' @export
make_table_15 <- function(df,
                          denominator = NULL,
                          return_ard = TRUE,
                          id_var = "USUBJID",
                          arm_var = "TRT01A",
