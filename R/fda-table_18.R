#' FDA Table 18: Patients With Adverse Events by Female-Specific FDA Medical Query (Broad)
#'   and Preferred Term, Female Safety Population, Pooled Analyses
#'
#' @description
#' Creates FDA Table 18 showing patients with adverse events by female-specific FDA Medical Query
#' (Broad) and preferred term, restricted to the female safety population.
#'
...
#' )
#'
#' result <- make_table_18(df = adae, denominator = adsl, arm_var = "ARM")
#' result$table
#'
#' @importFrom dplyr filter all_of
#' @importFrom gtsummary tbl_hierarchical gather_ard
#' @export
make_table_18 <- function(df,
                          denominator = NULL,
                          return_ard = TRUE,
                          id_var = "USUBJID",
                          arm_var = "TRT01A",
