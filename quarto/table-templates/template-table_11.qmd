...
# Output Table
make_table_11(adae = adae, alt_counts_df = adsl)
```
...
-   **`adae`**: `SAFFL`, `USUBJID`, `AEBODSYS`, `AESER`, `DCSREAS` and the variables specified by `arm_var`, `fmqsc_var`, and `fmqnam_var`.
...
| `fmq_scope`      | (`character`) FMQ scope, must be "NARROW" or "BROAD".                                                                                                                                                                                                  | `"NARROW"`    |
...
Source code for this function is available [here](https://github.com/pharmaverse/falcon/blob/main/R/fda-table_11.R).
:::
