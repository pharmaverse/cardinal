...
#### Parameter update
Each template takes several parameters that can be customized to generate a safety table that meets your requirements. A brief overview of the parameters is provided in the **Function Details** panel.

This is the example for table2:

**df**: The input dataset containing the variables to be summarized. 
**alt_counts_df**: An optional dataset containing alternative counts for each arm. 
**show_colcounts**: A logical value indicating whether to show the column counts in the table.
**arm_var**: The variable in the dataset that specifies the treatment arm.
**vars**: The variables in the dataset to be summarized.
**lbl_vars**: The labels for the variables specified by the vars parameter.
**lbl_overall**: The label for the overall population column.
**.stats**: The summary statistics to be calculated for each variable. The default is c("mean_sd", "median_range", "count_fraction").
**.formats**: The formats for the summary statistics. The default is NULL.
**na_rm**: A logical value indicating whether to remove rows with missing values.
**prune_0**: A logical value indicating whether to remove rows with all-zero values.
**annotations**: An optional list of annotations to be added to the table.

To customize the function, you can modify the values of these parameters as per your requirements, and run like below:
