...
test_that("Table 17 generation works with default values", {
  result <- make_table_17(adae, adsl)

  res <- expect_silent(result)
  expect_snapshot(res)
})

test_that("Table 17 generation works with custom values", {
  result <- make_table_17(
    adae,
    adsl,
    annotations = list(
      title = paste(
        "Table 17: Patients With Adverse Events by Female-Specific FDA Medical Query (Narrow) and Preferred Term,",
...
test_that("Table 17 generation works with NA values/pruned rows", {
  result <- make_table_17(adae, adsl, prune_0 = TRUE)

  res <- expect_silent(result)
  expect_snapshot(res)
})

test_that("Table 17 generation works with risk difference column", {
  risk_diff <- list(arm_x = "A: Drug X", arm_y = "C: Combination")
  result <- make_table_17(adae, adsl, risk_diff = risk_diff)
