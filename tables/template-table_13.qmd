---
title: "FDA Table 13"
subtitle: "Patients With Common Adverse Events Occurring at >=XX% Frequency, Safety Population, Pooled Analyses"
format: html
---

::: panel-tabset
## Spec Screenshot

![](images/table_13.png){fig-align="center"}

## Table

```{r, message=FALSE, warning=FALSE, echo=FALSE}
library(scda)
library(falcon)

adsl <- scda::synthetic_cdisc_dataset("rcd_2022_10_13", "adsl")
adae <- scda::synthetic_cdisc_dataset("rcd_2022_10_13", "adae")

make_table_13(adae = adae, alt_counts_df = adsl)
```

## Table Setup

```{r, eval=FALSE}
#| code-fold: show

# Load Libraries & Data
library(scda)
library(falcon)

adsl <- scda::synthetic_cdisc_dataset("rcd_2022_10_13", "adsl")
adae <- scda::synthetic_cdisc_dataset("rcd_2022_10_13", "adae")

# Output Table
make_table_13(adae = adae, alt_counts_df = adsl)
```

## Function Details

### `make_table_13()`
\
Source code for this function is available [here](https://github.com/pharmaverse/falcon/blob/main/R/fda-table_13.R).

**Required variables in `adae`:** `SAFFL`, `USUBJID`, and the variables specified by `pt_var` and `arm_var`.

| Argument         | Description                                                                                                                                                                                                                                             | Default                                             |
|:----------------|:--------------------------------------|:----------------|
| `adae`           | (`data.frame`) Data (typically ADAE) required to build table.                                                                                                                                                                                           | *No default*                                        |
| `alt_counts_df`  | (`character`) Alternative (full) data (typically ADSL) used only to calculate column counts.                                                                                                                                                            | `NULL`                                              |
| `show_colcounts` | (`flag`) Whether column counts should be printed.                                                                                                                                                                                                       | `TRUE`                                              |
| `min_freq`       | (`proportion`) Minimum % frequency (fraction of `N`) required in any column to include each row.                                                                                                                                                        | `0.05`                                              |
| `arm_var`        | (`character`) Arm variable used to split table into columns.                                                                                                                                                                                            | `"ARM"`                                             |
| `pt_var`         | (`character`) Preferred term variable from `adae` to include in the table.                                                                                                                                                                              | `"AETERM"`                                          |
| `lbl_pt_var`     | (`character`) Label corresponding to preferred term variable `pt_var` to print in the table.                                                                                                                                                            | `formatters::var_labels(adae, fill = TRUE)[pt_var]` |
| `lbl_overall`    | (`character`) If specified, an overall column will be added to the table with the given value as the column label.                                                                                                                                      | `NULL`                                              |
| `annotations`    | (named `list` of `character`) List of annotations to add to the table. Valid annotations types are `title`, `subtitles`, `main_footer`, and `prov_footer.` Each name-value pair should use the annotation type as name and the desired string as value. | `NULL`                                              |
:::
